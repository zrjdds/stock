...
class Stockholm(object):
            ...
            if ('Data' in quote):  # Data字段中存储了股票所需要的所有数据
                try:
                    ...
                    for quote_data in quote['Data']:  # 遍历当前股票每一天的数据
                        if (quote_data['Volume'] != '000' or quote_data[
                            'Symbol'] in self.index_array):  # 如果成交量不等于0或股票不是大盘指数才能正常处理
                            d = {}
    ...
    def file_data_load(self):
        ...
        print("文件中数据加载" + str(len(all_quotes_data)) + "个股票完成... time cost: " + str(
            round(timeit.default_timer() - start)) + "s" + "\n")
        ...
        for quote in selected_quotes:
            ...
            for i in range(1, 11):  # 这里为什么用11
                ...
                day2day_profit = self.get_profit_rate(quote['Data'][target_idx]['Close'],
                                                      quote['Data'][target_idx + i]['Close'])  # 获取多日股价变化率
